<?php
// validando se o usuario é adm ou fundador para acessar
include("../../includes/valida_adm.php");

$titulo = "Mensalidade";
include("../../includes/inicio.php");
?>
    <div class="conteudo">
    <div class="titulo">
        <div class="txt-titulo">
            <h1>Mensalidade</h1>
            <p>Acompanhe o status do seu pagamento mensal</p>
        </div>
    </div>
<?php
$stmt = $conexao->prepare("
    SELECT id, cliente_id, valor, data_vencimento, url_pagamento
    FROM mensalidades
    WHERE cliente_id = ? AND status IN (1, 2)
      AND (
        DATEDIFF(data_vencimento, CURRENT_DATE) <= 5
      );
");
$stmt->bind_param("i", $_SESSION['cliente_id']);
$stmt->execute();
$resultado = $stmt->get_result();
$row_pendente = $resultado->fetch_assoc();
$stmt->close();

if ($resultado->num_rows > 0):
    ?>
    <!-- div da mensalidade pendente -->
    <div class="flex items-center justify-center w-full">
        <div class="flex flex-col justify-center items-center gap-3 bg-fundo-interface p-8 rounded-lg border border-borda shadow-md">
            <i class="bi bi-exclamation-triangle flex items-center justify-center text-3xl rounded-full w-12 h-12 bg-red-100 text-red-600"></i>
            <h2 class="text-2xl font-bold text-gray-900 mb-2">Mensalidade Pendente!</h2>
            <p class="text-gray-600 text-lg">Identificamos que a mensalidade ainda não foi paga.</p>
            <div class="bg-red-50 border border-red-200 rounded-lg p-6 mb-6">
                <div class="flex items-center justify-center mb-4">
                    <i class="bi bi-currency-dollar text-2xl text-red-600"></i>
                    <h3 class="text-lg font-semibold text-red-800">Detalhes da Mensalidade</h3>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm place-items-center">
                    <div>
                        <span class="text-red-700">Dias até vencer:</span>
                        <span>
                                <?php
                                $dataVencimentoStr = $row_pendente['data_vencimento'];

                                $dataVencimento = new DateTime($dataVencimentoStr);
                                $hoje = new DateTime('today');

                                $diferenca = $hoje->diff($dataVencimento);

                                if ($diferenca->invert == 1) {
                                    echo '<strong style="color: #ef4444;">Vencida</strong>';
                                } elseif ($diferenca->days == 0) {
                                    echo '<strong style="color: #f97316;">Vence hoje</strong>';
                                } else {
                                    $dias = $diferenca->days;
                                    $textoDias = ($dias == 1) ? 'dia' : 'dias';
                                    echo "Faltam " . $dias . " " . $textoDias;
                                }
                                ?>
                            </span>
                    </div>
                    <div>
                        <span class="text-red-700">Valor: </span>
                        <span class="text-red-900"><?= formatarPreco($row_pendente['valor']) ?></span>
                    </div>
                    <div>
                        <span class="text-red-700">Vencimento:</span>
                        <span class="text-red-900"><?= htmlspecialchars(formatarData($row_pendente['data_vencimento'])) ?></span>
                    </div>
                    <div>
                        <span class="text-red-700">Status:</span>
                        <span class="text-red-900">Pendente</span>
                    </div>
                </div>
            </div>
            <?php if (isset($row_pendente['url_pagamento']) && $row_pendente['url_pagamento'] != NULL): ?>
                <a class="cursor-pointer w-full bg-principal text-white py-4 rounded-lg font-semibold text-lg hover:bg-principal-hover focus:ring-2 focus:ring-principal focus:ring-offset-2 transition-colors flex items-center justify-center space-x-3 shadow-lg"
                   href="<?= htmlspecialchars($row_pendente['url_pagamento']) ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 50 50"
                         class="stroke-white fill-white">
                        <path d="M 25 0.046875 C 22.924964 0.046875 20.850972 0.83457408 19.273438 2.4121094 L 2.4121094 19.271484 C -0.7429612 22.426555 -0.7429612 27.571493 2.4121094 30.726562 L 19.273438 47.587891 C 22.42773 50.742184 27.57227 50.742184 30.726562 47.587891 L 47.587891 30.728516 C 50.742961 27.573445 50.742961 22.428507 47.587891 19.273438 L 30.728516 2.4121094 C 29.15098 0.83457408 27.075036 0.046875 25 0.046875 z M 25 2.0332031 C 26.558964 2.0332031 28.118988 2.6307072 29.314453 3.8261719 L 38.486328 13 L 37.070312 13 C 35.479355 13 33.953288 13.631896 32.828125 14.755859 A 1.0001 1.0001 0 0 0 32.828125 14.757812 L 26.060547 21.525391 C 25.466839 22.119099 24.532404 22.119686 23.9375 21.525391 L 17.169922 14.757812 C 16.046276 13.632967 14.520644 13 12.929688 13 L 11.511719 13 L 20.6875 3.8261719 C 21.882965 2.6307072 23.441036 2.0332031 25 2.0332031 z M 9.5117188 15 L 12.929688 15 C 13.99073 15 15.007506 15.420769 15.755859 16.169922 A 1.0001 1.0001 0 0 0 15.755859 16.171875 L 22.523438 22.939453 C 23.882532 24.297158 26.116318 24.297745 27.474609 22.939453 L 34.242188 16.171875 C 34.993023 15.421792 36.00927 15 37.070312 15 L 40.486328 15 L 46.173828 20.6875 C 48.564758 23.078429 48.564758 26.923524 46.173828 29.314453 L 40.488281 35 L 37.070312 35 C 36.00927 35 34.993023 34.578161 34.242188 33.828125 L 27.474609 27.060547 C 26.795464 26.381401 25.897789 26.042986 25 26.042969 C 24.102211 26.042952 23.202984 26.381695 22.523438 27.060547 L 15.755859 33.828125 A 1.0001 1.0001 0 0 0 15.755859 33.830078 C 15.007506 34.579184 13.99073 35 12.929688 35 L 9.5136719 35 L 3.8261719 29.3125 C 1.4352424 26.921571 1.4352424 23.076476 3.8261719 20.685547 L 9.5117188 15 z M 25 28.029297 C 25.382185 28.02937 25.763693 28.177755 26.060547 28.474609 L 32.828125 35.242188 A 1.0001 1.0001 0 0 0 32.828125 35.244141 C 33.953288 36.368057 35.479355 37 37.070312 37 L 38.488281 37 L 29.3125 46.173828 C 26.922793 48.563535 23.077207 48.563535 20.6875 46.173828 L 11.513672 37 L 12.929688 37 C 14.520644 37 16.046276 36.367033 17.169922 35.242188 L 23.9375 28.474609 C 24.234952 28.177462 24.617815 28.029224 25 28.029297 z"></path>
                    </svg>
                    <span>Pagar com Pix</span>
                </a>
            <?php else: ?>
                <form action="../../backend/mensalidade/gerar_pagamento.php" method="POST" class="w-full">
                    <input type="hidden" name="mensalidade_id" value="<?php echo $row_pendente['id']; ?>">
                    <input type="hidden" name="valor" value="<?php echo $row_pendente['valor']; ?>">
                    <button class="cursor-pointer w-full bg-principal text-white py-4 rounded-lg font-semibold text-lg hover:bg-principal-hover focus:ring-2 focus:ring-principal focus:ring-offset-2 transition-colors flex items-center justify-center space-x-3 shadow-lg"
                            type="submit">
                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20"
                             viewBox="0 0 50 50" class="stroke-white fill-white">
                            <path d="M 25 0.046875 C 22.924964 0.046875 20.850972 0.83457408 19.273438 2.4121094 L 2.4121094 19.271484 C -0.7429612 22.426555 -0.7429612 27.571493 2.4121094 30.726562 L 19.273438 47.587891 C 22.42773 50.742184 27.57227 50.742184 30.726562 47.587891 L 47.587891 30.728516 C 50.742961 27.573445 50.742961 22.428507 47.587891 19.273438 L 30.728516 2.4121094 C 29.15098 0.83457408 27.075036 0.046875 25 0.046875 z M 25 2.0332031 C 26.558964 2.0332031 28.118988 2.6307072 29.314453 3.8261719 L 38.486328 13 L 37.070312 13 C 35.479355 13 33.953288 13.631896 32.828125 14.755859 A 1.0001 1.0001 0 0 0 32.828125 14.757812 L 26.060547 21.525391 C 25.466839 22.119099 24.532404 22.119686 23.9375 21.525391 L 17.169922 14.757812 C 16.046276 13.632967 14.520644 13 12.929688 13 L 11.511719 13 L 20.6875 3.8261719 C 21.882965 2.6307072 23.441036 2.0332031 25 2.0332031 z M 9.5117188 15 L 12.929688 15 C 13.99073 15 15.007506 15.420769 15.755859 16.169922 A 1.0001 1.0001 0 0 0 15.755859 16.171875 L 22.523438 22.939453 C 23.882532 24.297158 26.116318 24.297745 27.474609 22.939453 L 34.242188 16.171875 C 34.993023 15.421792 36.00927 15 37.070312 15 L 40.486328 15 L 46.173828 20.6875 C 48.564758 23.078429 48.564758 26.923524 46.173828 29.314453 L 40.488281 35 L 37.070312 35 C 36.00927 35 34.993023 34.578161 34.242188 33.828125 L 27.474609 27.060547 C 26.795464 26.381401 25.897789 26.042986 25 26.042969 C 24.102211 26.042952 23.202984 26.381695 22.523438 27.060547 L 15.755859 33.828125 A 1.0001 1.0001 0 0 0 15.755859 33.830078 C 15.007506 34.579184 13.99073 35 12.929688 35 L 9.5136719 35 L 3.8261719 29.3125 C 1.4352424 26.921571 1.4352424 23.076476 3.8261719 20.685547 L 9.5117188 15 z M 25 28.029297 C 25.382185 28.02937 25.763693 28.177755 26.060547 28.474609 L 32.828125 35.242188 A 1.0001 1.0001 0 0 0 32.828125 35.244141 C 33.953288 36.368057 35.479355 37 37.070312 37 L 38.488281 37 L 29.3125 46.173828 C 26.922793 48.563535 23.077207 48.563535 20.6875 46.173828 L 11.513672 37 L 12.929688 37 C 14.520644 37 16.046276 36.367033 17.169922 35.242188 L 23.9375 28.474609 C 24.234952 28.177462 24.617815 28.029224 25 28.029297 z"></path>
                        </svg>
                        <span>Pagar com Pix</span>
                    </button>
                </form>
            <?php endif; ?>
        </div>
    </div>
<?php else : ?>
    <?php
    $stmt = $conexao->prepare("SELECT data_pagamento, valor FROM mensalidades WHERE cliente_id = ? AND status = 0 ORDER BY data_pagamento DESC LIMIT 1 ");
    $stmt->bind_param("i", $_SESSION['cliente_id']);
    $stmt->execute();
    $resultado = $stmt->get_result();
    $row_pago = $resultado->fetch_assoc();
    $stmt->close();
    ?>
    <?php if ($row_pago): ?>
    <!-- div da mensalidade paga -->
    <div class="flex items-center justify-center w-full">
    <div class="flex flex-col justify-center items-center gap-3 bg-fundo-interface p-8 rounded-lg border border-borda shadow-md">
        <i class="bi bi-check-lg flex items-center justify-center text-4xl rounded-full w-12 h-12 bg-green-200 text-green-600"></i>
        <h2 class="text-2xl font-bold text-gray-900 mb-2">Você está em dia!</h2>
        <p class="text-gray-600 text-lg">Agradecemos por manter seu pagamento em dia.</p>
        <div class="bg-green-50 border border-green-200 rounded-lg p-6 mb-6 flex flex-col justify-center items-center">
            <div class="flex items-center space-x-2 mb-4">
                <i class="bi bi-check2-circle text-2xl text-green-600"></i>
                <h3 class="text-lg font-semibold text-green-800"> Status do Pagamento</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 place-items-center">
                <div>
                    <span class="text-green-700">Proximo Vencimento: </span>
                    <span class="text-green-900">
                        <?php
                        // buscando a data da proxima fatura
                        $stmt = $conexao->prepare("SELECT data_vencimento FROM mensalidades WHERE cliente_id = ? AND status = 1 ORDER BY data_vencimento DESC LIMIT 1");
                        $stmt->bind_param("i", $_SESSION['cliente_id']);
                        $stmt->execute();
                        $stmt->bind_result($data_vencimento);
                        $stmt->fetch();
                        $stmt->close();

                        echo htmlspecialchars(formatarData($data_vencimento));
                        ?>
                    </span>
                </div>
                <div>
                    <span class="text-green-700">Valor: </span>
                    <span class="text-green-900"><?= htmlspecialchars(formatarPreco($row_pago['valor'])); ?></span>
                </div>
                <div>
                    <span class="text-green-700">Último Pagamento: </span>
                    <span class="text-green-900"><?= htmlspecialchars(formatarData($row_pago['data_pagamento'])) ?></span>
                </div>
                <div>
                    <span class="text-green-700">Status: </span>
                    <span class="text-green-900">✓ Em Dia</span>
                </div>
            </div>
        </div>
    </div>
    <?php else: ?>
        <!-- div caso nenhuma mensalidade for encontrada -->
        <p>Nenhuma mensalidade encontrada.</p>
    <?php endif; ?>
<?php endif; ?>
<?php include("../../includes/fim.php"); ?>